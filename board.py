...
white_piece = colored('●', 'blue')
white_legal_move = colored('●', 'blue', attrs=['blink'])
black_piece = colored('●', 'white')
black_legal_move = colored('●', 'white', attrs=['blink'])
...
def print_board(board):
    ...
    for i in range(0, 8):
        ...
        # Afficher les scores
        if i == 2:
            row += colored('	   Score | ', 'red') + colored('Black: ' + str(score_black), 'blue')
        if i == 3:
            row += colored('	         | ', 'red') + 'White: ' + str(score_white)
        print(row)
        if not i == 7:
